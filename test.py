...
for i in range(testing_size):
    movie = Movie(idP=i, name=movies_df['title'][i], description=movies_df['description'][i], types=movies_df['listed_in'][i].split(', '), nlp=nlp)
    movies.append(movie)

# Create user instance
user = User(idU=1, login="driss", passw="pass")
